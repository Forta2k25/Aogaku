...
function tokyoMonthKey(date = new Date()) {
    const [year, month] = tokyoDateKey(date).split("-");
    return `${year}-${month}`;
}
function quotaCounterRef(uid, counterId) {
...
async function consumeTranscriptionQuota(uid) {
    ...
    return db.runTransaction(async (tx) => {
        ...
        if (currentCount >= transcriptionMonthlyLimit) {
            throw new functions.https.HttpsError("resource-exhausted", `今月のAI文字起こしは上限(${transcriptionMonthlyLimit}回)に達しました。`);
        }
...
async function refundTranscriptionQuota(uid, monthKey) {
    const ref = quotaCounterRef(uid, `transcription_${monthKey}`);
...
async function consumeReactionPaperQuota(uid) {
    ...
    return db.runTransaction(async (tx) => {
        ...
        if (currentCount >= reactionPaperDailyLimit) {
            throw new functions.https.HttpsError("resource-exhausted", `本日の生成回数が上限(${reactionPaperDailyLimit}回)に達しました。明日またお試しください。`);
        }
        const nextCount = currentCount + 1;
        tx.set(ref, {
            kind: "reactionPaper",
...
async function refundReactionPaperQuota(uid, dateKey) {
    const ref = quotaCounterRef(uid, `reactionPaper_${dateKey}`);
    await ref.set({
...
function shouldRetryWithAccurateModel(result) {
    const text = result.text?.trim() ?? "";
    if (!text || hasSuspiciousRepetition(text))
        return true;
    const speechSegments = (result.segments ?? []).filter((segment) => (segment.text?.trim().length ?? 0) > 0 && (segment.no_speech_prob ?? 0) < 0.8);
    if (speechSegments.length === 0)
        return false;
    const lowConfidenceCount = speechSegments.filter((segment) => Number.isFinite(segment.avg_logprob) && segment.avg_logprob < -1.15).length;
    const compressedCount = speechSegments.filter((segment) => Number.isFinite(segment.compression_ratio) &&
        segment.compression_ratio > 2.4).length;
    return lowConfidenceCount / speechSegments.length >= 0.5 || compressedCount > 0;
}
function summarizeTranscriptionQuality(result) {
    const segments = result.segments ?? [];
    const speechSegments = segments.filter((segment) => (segment.text?.trim().length ?? 0) > 0 && (segment.no_speech_prob ?? 0) < 0.8);
    const logProbabilities = speechSegments
        .map((segment) => segment.avg_logprob)
        .filter((value) => Number.isFinite(value));
    const lowConfidenceCount = speechSegments.filter((segment) => Number.isFinite(segment.avg_logprob) && segment.avg_logprob < -1.15).length;
    const highCompressionSegmentCount = speechSegments.filter((segment) => Number.isFinite(segment.compression_ratio) &&
        segment.compression_ratio > 2.4).length;
    const highNoSpeechSegmentCount = segments.filter((segment) => (segment.text?.trim().length ?? 0) > 0 && (segment.no_speech_prob ?? 0) >= 0.8).length;
    return {
...
function aggregateQuality(summaries) {
    const weightedLogProbability = summaries.reduce((acc, summary) => {
        if (summary.averageLogProbability === null)
            return acc;
        return {
    ...
    return {
        ...
        suspiciousSegmentCount: summaries.reduce((sum, summary) => sum + summary.suspiciousSegmentCount, 0),
        averageLogProbability: weightedLogProbability.count > 0
            ? weightedLogProbability.sum / weightedLogProbability.count
            : null,
        highNoSpeechSegmentCount: summaries.reduce((sum, summary) => sum + summary.highNoSpeechSegmentCount, 0),
        highCompressionSegmentCount: summaries.reduce((sum, summary) => sum + summary.highCompressionSegmentCount, 0),
    };
}
function textAfterOverlap(result, overlapSeconds) {
    const segments = result.segments ?? [];
...
async function transcribeWithGroq(signedUrl, model, contextPrompt, apiKey) {
...
async function transcribeWithOpenAI(signedUrl, contextPrompt, apiKey) {
...
async function cleanTranscriptSpelling(rawTranscript, contextPrompt, apiKey) {
 ...
 * 音声は処理後に必ず削除する。
 */
exports.transcribeLectureAudio = functions
    .region(region)
    ...
    const requestedPaths = Array.isArray(data.storagePaths)
        ? data.storagePaths.map((value) => String(value))
    ...
    const contextPrompt = truncate(String(data.contextPrompt ?? ""), transcriptionContextMaxLength)
        .replace(/\s+/g, " ")
        .trim();
    const allowedPath = new RegExp(`^users/${uid.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}/transcriptionUploads/[A-Za-z0-9-]+\\.m4a$`);
    if (storagePaths.length === 0 ||
        storagePaths.length > transcriptionMaxChunkCount ||
        storagePaths.some((storagePath) => !allowedPath.test(storagePath))) {
    ...
    if (durationSeconds <= 0 || durationSeconds > transcriptionMaxDurationSeconds) {
        throw new functions.https.HttpsError("invalid-argument", "録音は1回90分以内にしてください");
    }
    const files = storagePaths.map((storagePath) => admin.storage().bucket().file(storagePath));
    ...
    try {
        ...
        const turboQualitySummaries = [];
        const finalQualitySummaries = [];
        for (let index = 0; index < files.length; index += 1) {
            ...
            try {
                ...
                if (shouldRetryWithAccurateModel(result)) {
                    ...
                    if (accurateQuality.suspiciousSegmentCount < turboQuality.suspiciousSegmentCount ||
                ...
                if (!openAIApiKey) {
                    throw new functions.https.HttpsError("resource-exhausted", "AI文字起こしが混み合っています。しばらくしてからもう一度お試しください");
                }
        ...
        await Promise.all(files.map((file, index) => file.delete({ ignoreNotFound: true }).catch((error) => {
 ...
 */
exports.generateReactionPaper = functions
    .region(region)
    ...
    if (!transcript && !photoText) {
        throw new functions.https.HttpsError("invalid-argument", "文字起こしまたは撮影内容が必要です");
    }
    const uid = context.auth.uid;
